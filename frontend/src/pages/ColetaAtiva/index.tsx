...
import MapaDeslocamento from './components/MapaDeslocamento';
import AvaliacaoResiduos from './components/AvaliacaoResiduos';

type EstadoPagina = 'deslocamento' | 'avaliacao' | 'entrega';

export default function ColetaAtiva() {
  const navigate = useNavigate();
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down('sm'));
  const { getCategoriaById } = useCategorias();
  
  const [estado, setEstado] = useState<EstadoPagina>('deslocamento');
  ...
  // Buscar coleta ativa ou pendente
  const buscarColeta = useCallback(async () => {
    setLoading(true);
    setError(null);

    try {
      // Tenta buscar coleta em andamento primeiro
      let coletaAtiva = await coletaService.buscarColetaAtiva();
      
      // Se não houver coleta em andamento, tenta buscar pendente
      if (!coletaAtiva) {
        coletaAtiva = await coletaService.buscarColetaPendente();
      }

      if (!coletaAtiva) {
        // Nenhuma coleta encontrada, redireciona
        navigate('/coletas');
        return;
      }

      setColeta(coletaAtiva);
      
      // Buscar detalhes dos resíduos primeiro
      const detalhes = await buscarDetalhesResiduos(coletaAtiva.residuos_id);
      
      // Define o estado baseado no estado da coleta e dos resíduos
      if (coletaAtiva.estado === 'EM_ANDAMENTO') {
  ...
  const buscarDetalhesResiduos = async (residuosIds: string[]): Promise<ResiduoDetalhado[]> => {
    try {
      // Usa o serviço que chama a rota específica para coletores
      ...
      <>
        ...
        <Container maxWidth="lg" sx={{ py: 4 }}>
          <Alert severity="error" sx={{ mb: 2 }}>
            {error || 'Nenhuma coleta ativa encontrada.'}
          </Alert>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Typography variant="body1" color="text.secondary">
            {estado === 'deslocamento'
              ...
              : estado === 'entrega'
                ? 'Selecione os resíduos coletados que serão entregues e finalize a coleta'
