...
@pytest.mark.asyncio
async def test_generate_report_receptor_returns_empty(mock_user_repo):
    # Arrange: usuário receptora
    receptor_id = "receptor123"
    with patch('src.service.user_service.categoria_repo'):
        # categoria_repo not needed for receptor case
        mock_user_repo.find_by_id.return_value = {
            "_id": receptor_id,
            "role_id": "receptor",
            "name": "Ponto Receptor",
