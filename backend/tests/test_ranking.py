...
def _make_user(i, points, cidade="teresina", estado="pi"):
	return {
		"_id": f"user{i}",
		"name": f"User {i}",
		"cidade_id": cidade,
...
class TestRankingService:
	...
	async def test_get_top10_global(self):
		# Cria 12 usuários com pontos decrescentes (0..11)
		users = [ _make_user(i, points=100 - i) for i in range(12) ]
		...
		with patch('src.service.ranking_service.ranking_repo') as mock_repo:
			...
			mock_repo.get_users_sorted_by_points = AsyncMock(return_value=users[:10])
			...
			# Primeiro usuário deve ter o maior número de pontos (user0 => 100)
			assert response.top[0].user_id == 'user0'
			assert response.top[0].points == 100

	@pytest.mark.asyncio
	async def test_get_top10_by_state(self):
		# Usuários mistos em diferentes estados
		users = [ _make_user(i, points=50 - i, estado='pi') for i in range(5) ] + [ _make_user(i + 100, points=40 - i, estado='ma') for i in range(5) ]

		with patch('src.service.ranking_service.ranking_repo') as mock_repo:
			# ranking_repo deve retornar apenas usuários do estado solicitado
			mock_repo.get_users_sorted_by_points = AsyncMock(return_value=[u for u in users if u['estado_id'] == 'pi'])
	...
	async def test_get_top10_by_city(self):
		users = [ _make_user(i, points=10 + i, cidade='teresina') for i in range(3) ]
		users += [ _make_user(i + 100, points=5 + i, cidade='outra') for i in range(3) ]

		with patch('src.service.ranking_service.ranking_repo') as mock_repo:
			mock_repo.get_users_sorted_by_points = AsyncMock(return_value=[u for u in users if u['cidade_id'] == 'teresina'])
	...
	async def test_get_user_position_estado(self):
		users = [ _make_user(i, points=200 - i, estado='pi') for i in range(5) ]

		with patch('src.service.ranking_service.ranking_repo') as mock_repo:
			mock_repo.get_users_sorted_by_points = AsyncMock(return_value=users)
	...
	async def test_get_user_position_global_and_not_found(self):
		users = [ _make_user(i, points=200 - i) for i in range(5) ]
